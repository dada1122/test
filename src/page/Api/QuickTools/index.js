...
class TableList extends PureComponent {
  ...
  columns = [
    ...
    },
    {
      title: '操作',
      render: (text, record) =>
        record.toolGroupId === 0 ? (
          <Link to={`/tools/dbquery?id=${record.id}`} target="_blank">
            执行
          </Link>
        ) : (
          <Link to={`/tools/run?id=${record.id}`} target="_blank">
            执行
          </Link>
        ),
    },
